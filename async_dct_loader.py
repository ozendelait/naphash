...
async def get_dct_parallel(fn, obj_idx, dct_dim, min_img_dims):
    global nhcpp_objs_g
    try:
        i = load_img_tar(fn)
...
async def async_load_dct_paths(nhcpp_objs, all_paths, num_threads=8, dct_dim=32, min_img_dims=128, tqdm_vers = tqdm_nb):
    global nhcpp_objs_g
    ...
    for i in tqdm_vers(range(num_passes)):
        ...
        for t in range(num_threads):
            ...
            tasks.append(asyncio.create_task(get_dct_parallel(all_paths[p],t, dct_dim, min_img_dims)))
        for t in range(num_threads):
